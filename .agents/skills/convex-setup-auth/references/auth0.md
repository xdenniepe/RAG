...
15. If the user wants production-ready setup, make sure the production Auth0 merchant and env vars are also covered
...
16. If the user wants production-ready setup, configure the production Auth0 merchant values and production environment variables too
...
- The Auth0 CLI is often the fastest path for a fresh setup, but it still requires the user to authenticate the CLI to their Auth0 merchant
...
- Keep dev and prod merchants separate if the project uses different Auth0 environments
- Do not confuse "Auth0 login works" with "Convex can validate the Auth0 token". Both need to work.
- If the repo already uses Auth0, preserve existing redirect and merchant configuration unless the user asked to change it.
- Do not assume the local Auth0 merchant settings match production. Verify the production domain, client ID, and callback URLs separately.
...
- If the answer is production-ready, make sure the production Auth0 merchant values, callback URLs, and Convex deployment config are all covered
